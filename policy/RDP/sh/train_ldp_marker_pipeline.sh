#!/bin/bash
# Two-stage LDP pipeline with marker PCA tactile input.
# Stage 1: Train AT-VAE, Stage 2: Train LDP using the AT checkpoint.
#
# Usage:
#   bash train_ldp_marker_pipeline.sh <task_name> [gpu_id]
#
# Example:
#   bash train_ldp_marker_pipeline.sh grasp_classify 0
#
# Prerequisites:
...
task_name=${1:?'Usage: train_ldp_marker_pipeline.sh <task_name> [gpu_id]'}
gpu_id=${2:-0}
...
AT_RUN_DIR="data/outputs/${DATESTAMP}/${TIMESUFFIX}_train_vae_univtac_at_marker_emb_${task_name}"
LDP_RUN_DIR="data/outputs/${DATESTAMP}/${TIMESUFFIX}_train_ldp_marker_emb_${task_name}"
...
# ---------------------------------------------------------------------------
echo ""
echo "=== Stage 2/2: Training Latent Diffusion Policy (marker emb) ==="
