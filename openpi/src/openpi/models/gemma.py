...
class RMSNorm(nn.Module):
    ...
    def __call__(self, x, cond):
        ...
        # `cond` may be either per-batch (b, emb) -> single noise level shared across all
        # sequence positions (standard pi0/pi05), or per-position (b, s, emb) -> independent
        # noise level per token (diffusion forcing). Insert a singleton sequence axis only in
        # the per-batch case so the result broadcasts over the sequence dimension as before.
